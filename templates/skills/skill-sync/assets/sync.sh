...
# Usage: ./sync.sh [--dry-run] [--check] [--scope <scope>]
...
# Fallback: if skills doesn't exist at repo root, try templates/skills
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --scope)
            ...
            ;;
        --help|-h)
            echo "Usage: $0 [--dry-run] [--check] [--scope <scope>]"
            echo ""
            echo "Options:"
            echo "  --dry-run    Show what would change without modifying files"
            echo "  --check      Check if files are in sync (exit 1 if changes needed)"
...
# Track if changes are needed (for --check mode)
CHANGES_NEEDED=0
...
| $action | \`$skill_name\` |"
    ...
    if $DRY_RUN; then
        # Check if current content differs from what we would write
        current_section=$(awk '
            /^### Auto-invoke Skills/ { in_section=1 }
            in_section { print }
            in_section && /^(---|## )/ { exit }
        ' "$agents_path" 2>/dev/null || echo "")

        if [ "$current_section" != "$auto_invoke_section" ]; then
            CHANGES_NEEDED=1
            if $CHECK_MODE; then
                echo -e "${YELLOW}  ⚠ Out of sync: $agents_path${NC}"
            else
...
if $CHECK_MODE; then
    if [ $CHANGES_NEEDED -eq 0 ]; then
        echo ""
        echo -e "${GREEN}✅ All AGENTS.md files are in sync${NC}"
        exit 0
    else
        echo ""
        echo -e "${RED}❌ AGENTS.md files are out of sync${NC}"
        echo -e "${BLUE}Run './sync.sh' to update them${NC}"
        exit 1
    fi
fi