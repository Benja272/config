...
# Fallback: if templates/skills doesn't exist, use script dir (for copied setup.sh)
...
setup_copilot() {
    if [ -f "$REPO_ROOT/AGENTS.md" ]; then
        ...
        echo -e "${YELLOW}  Warning: No AGENTS.md found at repo root${NC}"
    fi
}
...
if [ "$SKILL_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}No skills found in $SKILLS_SOURCE${NC}"
    echo -e "${BLUE}Skills are optional - continuing with AGENTS.md setup...${NC}"
    echo ""
else
...
# =============================================================================
echo ""
echo -e "${GREEN}✅ AI assistant configuration complete!${NC}"
echo ""
echo "Configured:"
...
[ "$SETUP_CURSOR" = true ] && echo "  - Cursor:         .cursorrules"
echo ""
echo -e "${BLUE}Note: Restart your AI assistant to load the configuration.${NC}"
